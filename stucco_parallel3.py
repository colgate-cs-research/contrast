...
def update_time_measurement(time_taken):
    '''
    ...
    file = open("./learn_runtime_dataset.txt", "a")

    date_entry = "date:"+str(d4)+" | "
    L = date_entry+" get_rule_negations: "+str(time_taken)+"\n"
    file.writelines(L)
    file.close()
...
class ContrastSetLearner:
    ...
    def rule_negated_counts_helper(tt,emty=0):
        self,rule = tt
        time_toggle=False
        ...
        if time_toggle:
            ...
            update_time_measurement(execution_time)        # serves as an upper-bound for how many rules there could be
        
    def canonical_combination_helper(tt):
        self,max_length,num_states,rec = tt
        time_toggle=True
        ...
        for rule in canonical_combination(items, max_length):
            logging.debug('rule={}; state={}'.format(rule, state))
            ...
            self.counts[rule][0][pos] += count
        if time_toggle:
            execution_time = time.process_time()-start_time
            # updating time measurement in the 
            update_time_measurement(execution_time)   
    ...
    def learn(self, max_length=2, max_records=None, max_contingency_matrix=None):
        ...
        with concurrent.futures.ThreadPoolExecutor(max_workers=100) as executor:
            ...
            canonical_combination_futures =  executor.map(self.canonical_combination_helper, [(self,max_length,num_states,rec) for rec in self.data])
        logging.info("{:,} contingency matrices made".format(len(self.counts)))

        logging.info("\tC0mputing not-rules")
        ...
        for i, rule in enumerate(self.counts):
            ...
            for col_num in range(np.shape(contingency_matrix)[1]):
                ...
                if all(conditions):
                    ...
                    data.append(row)
                    logging.debug('{} / {}: {}'.format(i, len(self.counts), row))
                    logging.debug('{}'.format(two_by_two.tolist()))
